...
def box_plot_all_top(df_plot, x_label="Model\n", y_label="\nMax. Normalized Link Utilization", size=(30, 6), title=None, y_lim_top=None):
	...
	#plt.close()
	#print(file_name)
	plt.yticks([0,1],['JointUW','Weights'])
	plt.tight_layout()
	plt.grid()
	plt.show()
	return
...
data_joint   = read_csv_data( CSV_FILENAME, "SimpleJointUW.topo.sh")
...
print("JOINTUW Median: ", statistics.median(data_joint))
print("WEIGHTS Median: ", statistics.median(data_weights))
print("JOINTUW Minimum: ", min(data_joint))
print("WEIGHTS Minimum: ", min(data_weights))
print("JOINTUW Maximum: ", max(data_joint))
